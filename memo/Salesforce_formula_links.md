...
IMAGE("/sfc/servlet.shepherd/document/download/" & MID(Ids__c, 97, 18), MID(Labels__c, FIND(";5;", Labels__c) + 3, IF(FIND(";6;", Labels__c) = 0, LEN(Labels__c) + 1, FIND(";6;", Labels__c)) - FIND(";5;", Labels__c) - 3)),
HYPERLINK("/" & MID(Ids__c, 97, 18), MID(Labels__c, FIND(";5;", Labels__c) + 3, IF(FIND(";6;", Labels__c) = 0, LEN(Labels__c) + 1, FIND(";6;", Labels__c)) - FIND(";5;", Labels__c) - 3))),
"")
&
IF(LEN(Ids__c) >= 133,
" " & BR() &
IF(MID(Ids__c, 115, 1) = "I",
IMAGE("/sfc/servlet.shepherd/document/download/" & MID(Ids__c, 116, 18), MID(Labels__c, FIND(";6;", Labels__c) + 3, LEN(Labels__c) - FIND(";6;", Labels__c) - 2)),
HYPERLINK("/" & MID(Ids__c, 116, 18), MID(Labels__c, FIND(";6;", Labels__c) + 3, LEN(Labels__c) - FIND(";6;", Labels__c) - 2))),
"")